...
# loads and returns pm2.5 data of coal related emmisions
# (NEI, SCC)
loadData = function() {
    ...
    # filter coal related categories
    coalRelated = sectors[grep("Coal", sectors)]
    
    # filter coal related sources
    SCC = SCC[SCC$EI.Sector %in% coalRelated,]
    
    #filter coal related measurements 
...
# renders a plot of total coal related 2.5 emmision per year into plot4.png
plot4 = function() {
    data = loadData()
